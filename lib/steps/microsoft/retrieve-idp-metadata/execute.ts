...
export async function executeRetrieveIdpMetadata(
  ...
  try {
    ...
    const samlAppId = context.outputs[OUTPUT_KEYS.SAML_SSO_APP_ID] as
      | string
      | undefined;
    const spObjectId = context.outputs[OUTPUT_KEYS.SAML_SSO_SP_OBJECT_ID] as
      | string
      | undefined;
    ...
    if (!samlAppId || !spObjectId) {
      return {
        ...
        error: {
          message:
            "Missing required outputs: SAML_SSO_APP_ID or SAML_SSO_SP_OBJECT_ID. Ensure M-7 completed successfully.",
