...
export const executeConfigureSamlApp = withExecutionHandling({
  ...
  executeLogic: async (context: StepContext): Promise<StepExecutionResult> => {
    ...
    const googleSpEntityId = context.outputs[OUTPUT_KEYS.GOOGLE_SAML_SP_ENTITY_ID] as string;
    const googleAcsUrl = context.outputs[OUTPUT_KEYS.GOOGLE_SAML_SP_ACS_URL] as string;
