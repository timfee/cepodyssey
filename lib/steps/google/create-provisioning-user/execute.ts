import { googleApi, type DirectoryUser } from "@/lib/api/google/index";
...
export const executeCreateProvisioningUser = withExecutionHandling({
  ...
  executeLogic: async (context: StepContext): Promise<StepExecutionResult> => {
    ...
    try {
      result = await googleApi.users.create(user, context.logger);
    } catch (error) {
      if (error instanceof AlreadyExistsError) {
        const existing = await googleApi.users.get(email, context.logger);
